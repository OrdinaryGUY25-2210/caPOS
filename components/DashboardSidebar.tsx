...
export default function DashboardSidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    ...
    <aside className="w-72 sm:w-64 bg-sidebar flex flex-col h-full shrink-0">
      ...
      <nav data-tour="dashboard-nav" className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        data-tour={item.href === "/dashboard/settings" ? "settings-nav" : undefined}
                        className={cx(
