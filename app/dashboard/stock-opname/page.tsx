...
/**
 * Stok Opname — BAHAN BAKU (migration_16).
 *
 * Sejak migration_16, opname dilakukan per ingredient (bahan baku/komponen
 * resep), BUKAN per produk jadi. Ini menyatukan opname dengan sumber
 * kebenaran stok yang sesungguhnya dipakai HPP & pemotongan stok otomatis
 * saat checkout (branch_ingredients_stock via resep) — sebelum migration_16,
 * halaman ini membaca/menulis stok produk (branch_stock) yang untuk produk
 * olahan (made-to-order) sebetulnya sudah tidak relevan lagi sejak Phase 1
 * F&B Core ada, karena stok produk olahan murni mengikuti bahan baku.
 *
 * Jalur lama (submit_stock_opname / stock_opname_logs, berbasis products)
 * sudah dibekukan di database — RPC-nya sekarang selalu menolak dipanggil.
 */

interface IngredientStock extends Ingredient {
  cost_price: number;
}
...
export default function StockOpnamePage() {
  ...
  async function loadIngredients() {
    ...
    const [{ data: ingredients }, { data: stock }] = await Promise.all([
      supabase
        .from("ingredients")
        .select("*")
      ...
      supabase
        ...
        .select("ingredient_id, stock_qty, cost_price")
        .eq("branch_id", activeBranchId),
    ]);

    const stockMap = new Map<string, { qty: number; cost: number }>(
      (stock ?? []).map((s: any) => [s.ingredient_id, { qty: Number(s.stock_qty), cost: Number(s.cost_price) }])
    );
    ...
    setRows(
      ((ingredients as Ingredient[]) ?? []).map((ing) => {
        ...
        return {
          ingredient: { ...ing, cost_price: s?.cost ?? 0 },
          systemQty,
  ...
  function updateRow(ingredientId: string, patch: Partial<OpnameRow>) {
    setRows((prev) => prev.map((r) => (r.ingredient.id === ingredientId ? { ...r, ...patch } : r)));
  }
  ...
  async function submitRow(row: OpnameRow) {
    ...
    if (difference !== 0 && !row.reason) {
      alert(`Pilih alasan selisih untuk "${row.ingredient.name}" dulu sebelum disimpan.`);
      return;
    }

    updateRow(row.ingredient.id, { saving: true });
    const supabase = createClient();
    const { error } = await supabase.rpc("submit_stock_opname_item", {
      p_branch_id: activeBranchId,
      p_ingredient_id: row.ingredient.id,
    ...
    if (error) {
      updateRow(row.ingredient.id, { saving: false });
      alert("Gagal menyimpan opname: " + error.message);
      return;
    }

    updateRow(row.ingredient.id, {
      saving: false,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-neutral-900 flex items-center gap-2">
            <ClipboardList size={20} className="text-primary" /> Stok Opname Bahan Baku
          </h1>
          <p className="text-sm text-neutral-500 mt-0.5">
            Bandingkan stok fisik bahan baku dengan stok sistem — selisih otomatis dihitung nilainya berdasarkan HPP,
            dan koreksinya langsung memengaruhi kalkulasi HPP resep produk terkait.
          </p>
        </div>
        ...
        <>
          {dirtyCount > 0 && (
            <div className="rounded-xl bg-warning-light text-warning text-sm px-4 py-2.5">
              {dirtyCount} bahan baku punya selisih dari stok sistem — pilih alasan lalu klik Simpan per baris.
            </div>
          )}
                ...
                {!loading && rows.length === 0 && (
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-neutral-400">
                      Belum ada bahan baku aktif. Tambahkan bahan baku dulu di halaman Bahan Baku.
                    </td>
                  </tr>
                )}
                {rows.map((row) => {
                  const physical = row.physicalInput === "" ? 0 : Number(row.physicalInput);
                  const difference = physical - row.systemQty;
                  const lossPreview = difference < 0 ? Math.abs(difference) * row.ingredient.cost_price : 0;
                  const isDirty = difference !== 0;
                  return (
                    <tr key={row.ingredient.id} className={isDirty ? "bg-urgent-light/20" : undefined}>
                      <td className="p-3 font-medium text-neutral-900">
                        {row.ingredient.name}
                        <span className="ml-1.5 text-xs font-normal text-neutral-400">({row.ingredient.inventory_unit})</span>
                      </td>
                      ...
                      <td className="p-3">
                        <input
                          ...
                          onChange={(e) =>
                            updateRow(row.ingredient.id, { physicalInput: e.target.value.replace(/[^0-9]/g, ""), savedAt: null })
                          }
                      ...
                      <td className="p-3">
                        {isDirty ? (
                          <select
                            value={row.reason}
                            onChange={(e) => updateRow(row.ingredient.id, { reason: e.target.value })}
...
function OpnameHistory({ tenantId, branchId, showBranchColumn }: { tenantId: string; branchId: string | null; showBranchColumn: boolean }) {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const supabase = createClient();
      // v_stock_opname_history (migration_16) — view gabungan
      // stock_opnames + stock_opname_items + ingredients, menggantikan
      // query lama ke stock_opname_logs (yang sudah dibekukan/read-only).
      let q = supabase
        .from("v_stock_opname_history")
        .select("id, created_at, system_qty, physical_qty, difference_qty, loss_value, reason, note, unit, ingredient_name, branch_name, created_by_name")
