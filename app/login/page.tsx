...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Logika login Supabase dapat ditaruh di sini tanpa melakukan auto-redirect di useEffect
    try {
      // Pemanggilan supabase auth
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-zinc-300 dark:border-zinc-700 rounded-lg bg-transparent text-zinc-900 dark:text-zinc-100"
              required
            />
          </div>
          <div>
